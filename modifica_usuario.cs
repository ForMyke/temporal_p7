    ...
    public class modifica_usuario
    {
        ...
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "put")] HttpRequest req)
        {
            try
            {
                ...
                if (usuario == null) throw new Exception("Se esperan los datos del usuario");
                ...
                try
                {
                    ...
                    cmd_1.CommandText = "UPDATE usuarios SET email=@email,nombre=@nombre,apellido_paterno=@apellido_paterno,apellido_materno=@apellido_materno,fecha_nacimiento=@fecha_nacimiento,telefono=@telefono,genero=@genero WHERE id_usuario=@id_usuario";
                    ...
                    if (usuario.password != "")
                    {
                        ...
                        cmd_2.CommandText = "UPDATE usuarios SET password=@password WHERE id_usuario=@id_usuario";
                        cmd_2.Parameters.AddWithValue("@password", usuario.password);
                    ...
                    cmd_3.CommandText = "DELETE FROM fotos_usuarios WHERE id_usuario=@id_usuario";
                    cmd_3.Parameters.AddWithValue("@id_usuario", id_usuario);
                    ...
                    if (usuario.foto != null)
                    {
                        ...
                        cmd_4.CommandText = "INSERT INTO fotos_usuarios (foto,id_usuario) VALUES (@foto,@id_usuario)";
                        cmd_4.Parameters.AddWithValue("@foto", Convert.FromBase64String(usuario.foto));
                        cmd_4.Parameters.AddWithValue("@id_usuario", id_usuario);
                        cmd_4.ExecuteNonQuery();
                    }
                    transaccion.Commit();
                    return new OkObjectResult(JsonConvert.SerializeObject(new Respuesta("Se modificó el usuario")));
                }
                catch (Exception e)
                {
